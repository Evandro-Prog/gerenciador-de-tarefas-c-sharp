    ...
    public class TarefaController : BaseController
    {
        ...
        public IActionResult AdicionarTarefa([FromBody] Tarefa tarefa)
        {
            ...
            catch (Exception e)
            {
                ...
                return StatusCode(StatusCodes.Status500InternalServerError, new ErroRespostaDto()
                {
                    ...
                    Erro = "Ocorreu erro adicionar tarefa, tente novamente!"
                });
            }
        }
        ...
        public IActionResult Atualizartarefa([FromBody] Tarefa model, int idTarefa)
        {
            try
            {
                ...
                else if (!string.IsNullOrEmpty(model.Nome) && !string.IsNullOrWhiteSpace(model.Nome) && model.Nome.Length < 4)
                {
                    erros.Add("Favor informar um nome válido");
                }
            ...
            catch (Exception e)
            {
                ...
                {
                    Status = StatusCodes.Status500InternalServerError,
                    Erro = "Ocorreu erro ao atualizar, tente novamente!"
                });
            }
        }

        [HttpGet]
        public IActionResult ListarTarefasuUsuario(DateTime? periodoDe, DateTime? periodoAte, StatusTarefaEnum status)
        {
            try
            {
                var usuario = ReadToken();
